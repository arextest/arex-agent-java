 ...
 * @param <V>
 */
public class RFutureWrapper<V> implements RFuture<V> {

    private final CompletableFuture<V> future;
    private CompletableFuture<V> lastFuture;

    public RFutureWrapper(V value) {
        this(CompletableFuture.completedFuture(value));
    }

    public RFutureWrapper(Throwable ex) {
        this(new CompletableFuture<>());
        this.future.completeExceptionally(ex);
    }

    public RFutureWrapper(CompletionStage<V> stage) {
        this.future = stage.toCompletableFuture();
        this.lastFuture = future;
    }

    public RFutureWrapper(CompletableFuture<V> future) {
        this.future = future;
        this.lastFuture = future;
    }
