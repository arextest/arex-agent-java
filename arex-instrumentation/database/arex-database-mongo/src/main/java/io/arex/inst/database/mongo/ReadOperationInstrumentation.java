...
public class ReadOperationInstrumentation extends TypeInstrumentation {

    @Override
    protected ElementMatcher<TypeDescription> typeMatcher() {
        return namedOneOf("com.mongodb.operation.DistinctOperation",
    ...
    public List<MethodInstrumentation> methodAdvices() {
        return Collections.singletonList(new MethodInstrumentation(isMethod().and(named("execute")
                .and(takesArguments(1))), ReadOperationAdvice.class.getName()));
    }

    public static class ReadOperationAdvice{
        @Advice.OnMethodEnter(skipOn = Advice.OnNonDefaultValue.class, suppress = Throwable.class)
        public static boolean onEnter(@This Object thiz,
                                      @FieldValue(value = "namespace") MongoNamespace namespace,
                                      @FieldValue(value = "filter") Object filter,
                                      @Advice.Local("mockResult") MockResult mockResult) {
            if (ContextManager.needRecord()) {
                RepeatedCollectManager.enter();
            }
            if (ContextManager.needReplay()) {
                mockResult = MongoHelper.replay(thiz.getClass().getSimpleName(), namespace, filter);
