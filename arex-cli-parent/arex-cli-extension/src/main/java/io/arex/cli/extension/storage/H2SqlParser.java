...
public class H2SqlParser {
    ...
    public static Map<String, String> parseSchema() {
        try {
            String schemaSql = IOUtils.toString(
                    H2StorageImpl.class.getResourceAsStream("/db/h2/schema.txt"));
    ...
    public static String generateSelectSql(Mocker mocker, String tableName, int count) {
        StringBuilder sqlBuilder = new StringBuilder("SELECT * FROM ");
        switch (mocker.getCategoryName()) {
    ...
    public static String generateCompareSql(String category, String recordId, String replayId) {
        switch (category) {
            case "ServletEntrance":
    ...
    public static String generateSelectDiffSql(DiffMocker mocker) {
        StringBuilder sqlBuilder = new StringBuilder("SELECT * FROM ");
        String tableName = "DIFF_" + mocker.getCategory();
        sqlBuilder.append(tableName).append(" WHERE 1 = 1");
